/**
 * Re-catalogador de la vista maestra (TASK-004) y propose IA (TASK-010/020).
 *
 * En TASK-028 el drawer, la visibilidad y la configuración salieron de aquí a
 * módulos ES (asset/js/main.js). Lo que queda se engancha al drawer nuevo por
 * el evento `oer:drawer-rendered` y se irá en las tareas 13 y 14.
 */
(function ($) {
    'use strict';
    ...
    // El drawer ya no cablea el panel: avisa al pintarse y quien quiera añadir
    // algo se engancha aquí (TASK-028, transición hacia ui/recatalog.js).
    document.addEventListener('oer:drawer-rendered', function (e) {
        if (!canRecatalog()) {
            return;
        }
        $(e.detail.content).append(buildRecatalogPanel(e.detail.itemId, e.detail.itemJson));
    });
    ...
    $(document).on('click', '.oer-recatalog-apply', function () {
        ...
        $.post(url, $.param(pairs)).done(function (response) {
            if (response.updated) {
                document.dispatchEvent(new CustomEvent('oer:open-drawer', {
                    detail: { apiUrl: apiUrl, itemId: itemId }
                }));
                return;
            }
            var messages = {
