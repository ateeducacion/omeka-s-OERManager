...
2. **Auditoría de curación = RDF nativa vía value annotations.** Cada escritura de alineamiento/tag se anota sobre su propio valor con properties `dcterms`: **quién** (`dcterms:contributor`), **cuándo** (`dcterms:modified`), **qué** (`dcterms:provenance`). Sin módulo Log y sin tablas propias (respeta NFR-002). **Ampliación (TASK-023, 2026-07-08):** cuando la propuesta viene de la IA, el valor de saber (`lrmi:teaches`) o criterio (`lrmi:assesses`) puede llevar además el **«porqué»** de la elección como **`dcterms:description`** en el mismo `@annotation` (distinto del «qué» de `dcterms:provenance`), literal acotado a 200 chars. Es un valor más de la anotación, dentro de los vocabularios permitidos; opcional (solo si la IA aportó justificación).
