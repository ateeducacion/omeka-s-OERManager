...
- **Re-catalogador (TASK-004): en curso (2026-06-22/23).** Diseño aprobado en plan mode. Se entrega en dos fases: **4a** (manual: autocomplete curricular + tags, preview+confirmar+auditoría+reversibilidad, config form ADR-0006, ACL NFR-003) en TASK-004; **4b** (IA-assist) en TASK-010. Revisión en contexto fresco (2026-06-23) detectó y se corrigió el **borrado accidental** (ahora solo se envían dimensiones editadas) y se cambió el selector al widget **Chosen** nativo. Modelo de términos confirmado contra el contenedor (2026-06-24) y formalizado en **ADR-0009 (grafo curricular LOMLOE)**: la dimensión se distingue por **`dcterms:type`** y los nodos forman un grafo (Etapa←Curso←Asignatura←{Saber, Criterio}); el re-catalogador implementa **acotación contextual en cascada** (RF-014). Ver [referencia/curriculo-modelo-rdf.md](referencia/curriculo-modelo-rdf.md). Lote→Job (RF-011/TASK-011) y visión (RF-012/TASK-012), posteriores.
